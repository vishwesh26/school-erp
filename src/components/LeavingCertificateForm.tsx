...
const LeavingCertificateForm = ({ student }: { student: any }) => {
    ...
    const { register, handleSubmit, reset, watch } = useForm({
        defaultValues: {
            ...
            udise: SCHOOL_UDISE,
            studentName: `${student?.name || ""} ${student?.surname || ""}`.trim().toUpperCase(),
            motherName: (student?.motherName || "").toUpperCase(),
            fatherName: (student?.fatherName || (`${student?.parent?.name || ""} ${student?.parent?.surname || ""}`.trim().toUpperCase()) || "").toUpperCase(),
            aadharNo: student?.aadharNo || "",
            dobFigures: formatDateISO(student?.birthday),
            dobWords: "",
            ...
            dateApplication: formatDateISO(student?.dateApplication) || formatDateISO(new Date()),
            dateIssue: formatDateISO(student?.dateIssue) || formatDateISO(new Date()),
    ...
    useEffect(() => {
        reset({
            ...
            fatherName: student?.fatherName || (`${student?.parent?.name || ""} ${student?.parent?.surname || ""}`.trim().toUpperCase()) || "",
            aadharNo: student?.aadharNo || "",
            dobFigures: formatDateISO(student?.birthday),
            dobWords: "",
